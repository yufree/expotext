...
def resolve_month():
    ...
    if target:
        ...
        if not (1 <= month <= 12):
            raise ValueError(
                f"TARGET_MONTH month must be 1–12, got {month}"
            )
        if year < 1900 or year > 2100:
            raise ValueError(
                f"TARGET_MONTH year out of range, got {year}"
            )
...
def _http_request(url, params, *, parse_json=False):
    ...
    req = urllib.request.Request(
        full_url, headers={"User-Agent": "exposome-monthly-updater/1.0"}
    )
    last_exc = None
...
# Case-insensitive month abbreviation map
